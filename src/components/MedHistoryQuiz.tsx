...
export default function MedHistoryQuiz({ onComplete, onSkip }: Props) {
  ...
  const [formData, setFormData] = useState<QuizFormData>({
    ...
    allergies: [],
    additionalNotes: '',
    // N/A flags for each section
    personalInfoNA: false,
    currentSkinConcernsNA: false,
    familyHistoryNA: false,
    abcdeAssessmentNA: false,
    riskFactorsNA: false,
  ...
  const steps = [
    {
      ...
      fields: (
        <div className="space-y-4">
          <label className="flex items-center mb-4 p-2 bg-gray-50 rounded">
            <input
              type="checkbox"
              checked={formData.personalInfoNA}
              onChange={(e) => {
                setFormData({ 
                  ...formData, 
                  personalInfoNA: e.target.checked,
            ...
            />
            <span className="text-gray-700 font-medium">Not applicable / Prefer not to answer</span>
          </label>
          
          <div className={formData.personalInfoNA ? "opacity-50 pointer-events-none" : ""}>
            <div>
              ...
              <input
                ...
                disabled={formData.personalInfoNA}
              />
            </div>
      ...
      fields: (
        <div className="space-y-4">
          <label className="flex items-center mb-4 p-2 bg-gray-50 rounded">
            <input
              type="checkbox"
              checked={formData.currentSkinConcernsNA}
              onChange={(e) => {
                setFormData({ 
                  ...formData, 
                  currentSkinConcernsNA: e.target.checked,
                  // Clear fields if N/A is selected
                  ...(e.target.checked ? {
                    currentSkinConcerns: [],
                  } : {})
              ...
              className="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500 mr-2"
            />
            <span className="text-gray-700 font-medium">Not applicable / No current skin concerns</span>
          </label>
          
          <div className={formData.currentSkinConcernsNA ? "opacity-50 pointer-events-none" : ""}>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">Current Skin Concerns</label>
                ...
                {['Changing moles', 'New growths', 'Unexplained pigmentation'].map((concern) => (
                  <label key={concern} className="flex items-center">
                    <input
                      ...
                      disabled={formData.currentSkinConcernsNA}
                    />
        ...
        <div className="space-y-4">
          ...
          <div className={formData.familyHistoryNA ? "opacity-50 pointer-events-none" : ""}>
            ...
            {(formData.familyHistorySkinCancer || formData.familyHistoryMelanoma) && !formData.familyHistoryNA && (
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Affected Family Members</label>
                <div className="space-y-2">
      ...
      fields: (
        <div className="space-y-4">
          <label className="flex items-center mb-4 p-2 bg-gray-50 rounded">
            <input
              type="checkbox"
              checked={formData.abcdeAssessmentNA}
              onChange={(e) => {
                setFormData({ 
                  ...formData, 
                  abcdeAssessmentNA: e.target.checked,
                  // Clear fields if N/A is selected
                  ...(e.target.checked ? {
                    moleChanges: {
                      asymmetry: false,
              ...
              className="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500 mr-2"
            />
            <span className="text-gray-700 font-medium">Not applicable / No moles to assess</span>
          </label>
          
          <div className={formData.abcdeAssessmentNA ? "opacity-50 pointer-events-none" : ""}>
            ...
            <div className="space-y-2">
              <label className="flex items-center">
                <input
                  ...
                  className="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500"
                  disabled={formData.abcdeAssessmentNA}
                />
      ...
      fields: (
        <div className="space-y-4">
          <label className="flex items-center mb-4 p-2 bg-gray-50 rounded">
            <input
              type="checkbox"
              checked={formData.riskFactorsNA}
              onChange={(e) => {
                setFormData({ 
                  ...formData, 
                  riskFactorsNA: e.target.checked,
                  // Clear fields if N/A is selected
                  ...(e.target.checked ? {
                    outdoorOccupation: false,
                    tanningSalonUse: false,
          ...
          <div className={formData.riskFactorsNA ? "opacity-50 pointer-events-none" : ""}>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">Sun Exposure & Risk Factors</label>
              <div className="space-y-2">
                <label className="flex items-center">
                  <input
                    type="checkbox"
                    ...
                    className="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500"
                    disabled={formData.riskFactorsNA}
                  />
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700">Average Daily Sun Exposure</label>
              <select
